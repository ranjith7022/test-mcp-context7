...
## Local Context7 MCP Setup (No API Key)

This repository is configured to run Context7 MCP locally in:

- Cursor via `.cursor/mcp.json`
- VS Code / GitHub Copilot via `.vscode/mcp.json`
- OpenCode via `opencode.json`

All configs use local stdio mode and do not require an API key.
...
- Prompt naturally and add `use context7` when needed.
- Example: `use context7 for React Router v7 loader examples`.

### Client Notes

- Cursor: open Cursor MCP settings and ensure `context7` is running.
- VS Code/Copilot: open command palette, run `MCP: List Servers`, then start `context7` if needed.
- OpenCode: run your session in this project; OpenCode reads `opencode.json`.
